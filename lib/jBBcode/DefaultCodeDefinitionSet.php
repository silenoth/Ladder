...
class DefaultCodeDefinitionSet implements CodeDefinitionSet
{
    ...
    public function __construct()
    {
        ...
        /*[spoiler=Example]*/
        $builder = new CodeDefinitionBuilder('spoiler', '<div><a href="javascript:void(0)" onclick="if ($(this).parent().find(\'div:first\').is(\':hidden\')){$(this).parent().find(\'div:first\').slideDown(300);}else{$(this).parent().find(\'div:first\').slideUp(300);}">{option}</a><div name="spoiler" style="display:none;">{param}</div></div>');
        $builder->setUseOption(true)->setParseContent(true);
        array_push($this->definitions, $builder->build());

        /*[spoiler=Example]*/
        $builder = new CodeDefinitionBuilder('spoiler', '<div><a href="javascript:void(0)" onclick="if ($(this).parent().find(\'div:first\').is(\':hidden\')){$(this).parent().find(\'div:first\').slideDown(300);}else{$(this).parent().find(\'div:first\').slideUp(300);}"><b>Spolier</b></a><div name="spoiler" style="display:none;">{param}</div></div>');
        array_push($this->definitions, $builder->build());
